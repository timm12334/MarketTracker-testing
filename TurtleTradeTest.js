...
setInterval(function(){exec()}, 3600000);exec()
exec()
async function exec(){
    ...
    var report20=S1(MaxandMin10,MaxandMin20,dataset.c[periods],3)
    var report55=S2(MaxandMin10,MaxandMin20,dataset.c[periods],3)
    if(report20){
        SendMail(report20)
    }
    if(report55){
...
function S1(MaxandMin10,MaxandMin20,cp,addon){
    ...
    if(cp<MaxandMin20.min){ 
        ...
        for(i=0;i<addon;i++){
            addonP=(MaxandMin20.min-(dataset.atr[periods]/2)*(i+1)).toFixed(3)
            report.a=report.a.concat(addonP,', ')  
        }
        report.stop1='Stop at 10 days high: '+MaxandMin10.max.toFixed(3)
        report.stop2='Stop at 2ART: '+(dataset.c[periods]+dataset.atr[periods]*2).toFixed(3)
        return report
    }
...
function S2(MaxandMin20,MaxandMin55,cp,addon){
    //cp = currentprice, addon= times of followup position into market. 
    let report={}
    if(cp<MaxandMin55.min){ 
        ...
        for(i=0;i<addon;i++){
            addonP=(MaxandMin55.min-(dataset.atr[periods]/2)*(i+1)).toFixed(3)
            report.a=report.a.concat(addonP,', ')  
        }
...
function SendMail(report){
    ...
    const msg = {
        ...
        text: 'what!!',
        html: '<h>'+report.a+'<h>'+'<ul>'+'<li>'+report.stop1+'</li>'+'<li>'+report.stop2 +'</li>'+'</ul>',
      };
    console.log(msg)
    sgMail.send(msg);
    console.log('sent')

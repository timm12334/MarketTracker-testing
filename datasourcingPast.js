...
exec()
async function exec(){
    for(t=1451606400;t<currenttime;t+=604800){
    url= await UpdateUrl(t)
    dataset= await UpdateData(url)
    insertdata.insertdata('testing',dataset)
    console.log(dataset[0],' ',dataset[periods-1],'t:',t)
    //SendMailTopDivergence(dataset[periods-1].t)
    }
}
